...
const SECTORS: Array<RwandaSector | "All Sectors"> = ["Telecom", "Energy", "Water", "Transport", "All Sectors"];
const TRIGGERS: { value: WorkflowTrigger; label: string }[] = [
  { value: "license_application", label: "Licence Application" },
...
interface Props { open: boolean; onClose: () => void; onCreated: (w: Workflow) => void }

export function NewWorkflowModal({ open, onClose, onCreated }: Props) {
  const { toast } = useToast();
  const [loading, setLoading] = useState(false);
  const [form, setForm] = useState({
    name: "", description: "",
    trigger: "manual" as WorkflowTrigger,
    sector: "All Sectors" as RwandaSector | "All Sectors",
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.name) {
      toast("Workflow name is required.", "error"); return;
    }
    setLoading(true);
    try {
      const created = await createWorkflow({
        name: form.name, description: form.description,
        trigger: form.trigger, sector: form.sector, status: "draft",
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div className="w-full max-w-md rounded-2xl bg-card shadow-2xl"
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div>
                  <label className={LABEL}>Workflow Name <span className="text-danger">*</span></label>
                  <input className={INPUT} placeholder="e.g. Licence Renewal Review" value={form.name}
                    onChange={(e) => set("name", e.target.value)} disabled={loading} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className={LABEL}>Trigger</label>
                    <select className={SELECT} value={form.trigger}
                      onChange={(e) => set("trigger", e.target.value)} disabled={loading}>
                  ...
                  <div>
                    ...
                    <select className={SELECT} value={form.sector}
                      onChange={(e) => set("sector", e.target.value as RwandaSector | "All Sectors")} disabled={loading}>
                ...
                <div>
                  ...
                  <textarea className={INPUT} rows={3} placeholder="Describe the workflow purpose..."
